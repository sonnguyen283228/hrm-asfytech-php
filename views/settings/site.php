    ...
    <form method="post" action="/settings/site" enctype="multipart/form-data">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        <div>
          ...
          <div id="logoDrop" class="drop-zone">Kéo thả logo vào đây hoặc bấm để chọn file<input id="logoInput" type="file" name="site_logo_file" accept=".png,.jpg,.jpeg,.webp,.svg,.gif" style="display:none"></div>
        </div>
        <div>
          ...
          <div id="favDrop" class="drop-zone">Kéo thả favicon vào đây hoặc bấm để chọn file<input id="favInput" type="file" name="site_favicon_file" accept=".ico,.png,.svg" style="display:none"></div>
        </div>
      </div>
...
function setupDrop(zoneId, inputId){
  ...
  zone.addEventListener('drop',(e)=>{e.preventDefault(); zone.style.background='#f6f9ff'; if(e.dataTransfer.files?.[0]){input.files=e.dataTransfer.files; zone.firstChild.textContent='Đã chọn: '+e.dataTransfer.files[0].name+' ';}});
  input.addEventListener('change',()=>{if(input.files?.[0]) zone.firstChild.textContent='Đã chọn: '+input.files[0].name+' ';});
}
setupDrop('logoDrop','logoInput'); setupDrop('favDrop','favInput');
</script>
